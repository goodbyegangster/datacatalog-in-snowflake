...
- [x] **Step 1. 骨組みを画面に通す**：`streamlit_app.py` ＋ `views/` 2 枚を **モックデータ**で描画。
  1:3 レイアウト・`st.dataframe`・行選択・タブが出る状態にする。`lib/connection` `lib/catalog`
...
- **検索アコーディオンの開閉**：`st.expander` を採用（Streamlit 標準の範囲に留める方針）。
  現状は「その分類で一度でも選択したら開いたまま保持（ラッチ）、『入力をクリア』で閉じる」。
  - **保留の制約**：`st.expander` は `key` を持たず手動開閉状態を完全制御できないため、
    「**何も選択せず手動で開いただけのアコーディオンは『入力をクリア』でも閉じない**」
    エッジケースが残る（選択して開いた分は閉じる）。
