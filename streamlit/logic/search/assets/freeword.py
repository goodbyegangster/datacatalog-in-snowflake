...
def _find_asset_ids_matching_token(
    token: str, assets: pd.DataFrame, columns: pd.DataFrame, fq: FreewordQuery
) -> set[int]:
    """1 トークンに一致するデータ資産の TABLE_ID を返す。

    カラム名 / 説明に一致した場合は、そのカラムが属するデータ資産を含める。
    """
...
def _get_ordered_column_names(columns: pd.DataFrame, mask: pd.Series) -> list[str]:
...
def _find_match_reasons_by_token(
    token: str, assets: pd.DataFrame, columns: pd.DataFrame, fq: FreewordQuery
) -> dict[int, _FreewordMatchHit]:
    """1 トークンに一致した場所を TABLE_ID ごとに返す。"""
    assets_schema = schema.Assets
    columns_schema = schema.Columns
    needle = token.lower()
    ...
    if fq.target_column_name:
        ...
        for table_id, group in columns.loc[name_mask].groupby(columns_schema.TABLE_ID):
            reasons.setdefault(
                _convert_to_int(table_id), _FreewordMatchHit()
            ).column_names = _get_ordered_column_names(
    ...
    if fq.target_column_desc:
        ...
        for table_id, group in columns.loc[desc_mask].groupby(columns_schema.TABLE_ID):
            reasons.setdefault(
                _convert_to_int(table_id), _FreewordMatchHit()
            ).column_desc_names = _get_ordered_column_names(
                group, pd.Series(data=True, index=group.index)
            )
    return reasons
